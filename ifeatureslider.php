  ...
  class IfeatureSlider
  {
    ...
    public function hide_publishing_actions(){
      global $post;
            if($post->post_type == $this->shortcode){
                ...
                if($this->_slider) echo ' #post-body';
                else echo ' #misc-publishing-actions,#minor-publishing-actions';
                echo '{
                            display:none;
                        }';
                echo '</style>';
    ...
    */
    public function plugin_shortcode( $atts = array() )
    {
      ...
      if(!$slides) echo 'Invalid slider id';
      else
      {
        $mrtop                                          = 20;
        $this->s_options                                = $this->get_options();
        if(isset($this->s_options['s_auto'])) $auto     = 'auto';
        else                                  $auto     = '';
        $this->slider_count++;
    ...
    public function front_end_script()
    {
      ...
      <script>
        ...
        var _ifslider       = jQuery('.ifeatureslider');
            ...
            jQuery('#<?php echo $options['id']; ?>').iFeatureSlider({duration:<?php echo $options['duration']; ?>,delay:<?php echo $options['delay']; ?>,auto:<?php echo $options['auto']; ?>,imgs:<?php echo $options['count']; ?>});  
            <?php } ?>
      </script>
      <?php
    }
    ...
    public function update_slider_post()
    {
      ...
      if($post_type!==$this->shortcode)      return false;
      else  $this->if_slider_check();

      if(!isset($_POST['if_slider_imgs'])) return false;
      if(isset($_POST['save']) || isset($_POST['publish']))
      {
        ...
        if(count($array)>2) $array                      = array( $array[0], $array[1] );
    ...
    */
    ...
    public function render_options_meta_box( $post )
    {
      ...
      if(empty($opts)) $c = 'checked="checked"';
      ?>
      <div class="cc-main-content" >
        <p>
        <label>Slider Delay <small>(sec)</small></label>
        <input type="text" value="<?php echo $d; ?>" name="if_slider_options[s_delay]" />
        </p>
        <p>
        <label>Transition Time <small>(sec)</small></label>
...
<li class="cc-has-children  section-group if-slide" data-slide-type="image">
  <a class="if-header" href="#"><strong class="if-title"> Slide [<?php echo $slide['id']; ?>] </strong> <span class="if-controls"> <span class="if-delete" title="Delete"> Delete </span> </span>
    <div class="clear"></div>
  </a>
  <div class="if-body">
...
<?php
    ...
    public function admin_footer()
    {
      ?>
<li class="cc-has-children  section-group if-slide" data-slide-type="image">
  <a class="if-header" href="#"><strong class="if-title"> New Slide </strong> <span class="if-controls"> <span class="if-delete" title="Delete"> Delete </span> </span>
    <div class="clear"></div>
  </a>
  <div class="if-body">
    <div class="if-slide-image">
...
<?php
    ...
    {
      if(!isset($_GET['post_type']) || $_GET['post_type']!==$this->shortcode) return false;
    ...
    public function if_slider_admin_notice() 
    {
      ...
      {
        ?>
        <div class="error"><p><?php _e( 'Sorry, only one slider is possible in free version. Please upgrade your slider.', 'my-text-domain' ); ?></p>
        </div>
        <?php
      }
}
